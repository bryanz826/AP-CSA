...
public class GameOver implements State
{
	...
	public void render(Graphics2D g) {
		...
		if (win) new Text("YOU WIN", Reference.CENTER_X - 250, Reference.CENTER_Y - 250, 1.5f).drawString(g);
		else {
			if (rand == 0) new Text("MAYBE NEXT TIME", 120, Reference.CENTER_Y - 250, 1.5f).drawString(g);
			else if (rand == 1)
				new Text("GAME OVER", Reference.CENTER_X - 350, Reference.CENTER_Y - 250, 1.5f).drawString(g);
			else if (rand == 2)
				new Text("BAD LUCK", Reference.CENTER_X - 300, Reference.CENTER_Y - 250, 1.5f).drawString(g);
			else if (rand == 3) new Text("BAD COLLISION DETECTION???", 30, Reference.CENTER_Y - 210, 1).drawString(g);
		}

		new Text(options[0], Reference.CENTER_X + 150, Reference.CENTER_Y + 25, 0.85f).drawString(g);
		new Text(options[1], Reference.CENTER_X + 275, Reference.CENTER_Y + 150, 0.85f).drawString(g);
		...
		new Text("Score: ", 70, Reference.CENTER_Y + 70, 0.8f).drawString(g);
		;
		new Text("" + Play.getScore(), 320, Reference.CENTER_Y + 70, 0.8f).drawStringFixed(g, 6, "0");
		new Text("Time: ", 70, Reference.CENTER_Y + 150, 0.8f).drawString(g);
		new Text("" + Play.getTimer() / 3600, 320, Reference.CENTER_Y + 150, 0.8f).drawStringFixed(g, 2, "0");
		new Text(":", 400, Reference.CENTER_Y + 150, 0.8f).drawString(g);
		new Text("" + ((Play.getTimer() % 3600) / 60), 440, Reference.CENTER_Y + 150, 0.8f).drawStringFixed(g, 2, "0");
	}
